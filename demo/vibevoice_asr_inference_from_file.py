...
def _select_device(device: str) -> str:
    ...
    if requested == "xpu":
        if _xpu_available():
            return "xpu"
        if torch.cuda.is_available():
            print("Warning: XPU not available. Falling back to CUDA.")
            return "cuda"
        if torch.backends.mps.is_available():
            print("Warning: XPU not available. Falling back to MPS.")
            return "mps"
    ...
    if requested == "mps":
        if torch.backends.mps.is_available():
            return "mps"
        if _xpu_available():
            print("Warning: MPS not available. Falling back to XPU.")
            return "xpu"
...
def main():
    ...
    parser.add_argument(
        "--device",
        type=str,
        default="cuda" if torch.cuda.is_available() else ("xpu" if _xpu_available() else ("mps" if torch.backends.mps.is_available() else "cpu")),
        choices=["cuda", "xpu", "mps", "cpu", "auto"],
    ...
    selected_dtype = _select_dtype(selected_device)
